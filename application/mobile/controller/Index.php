...
class Index extends Globals{
    ...
    public function index(){
        ...
        //服务项目
        $server = db('nav')->where('category_id',82)->limit(4)->order('nav_id','asc')->select();
        ...
        //案例
        $doc = db('nav')->where('category_id',83)->limit(4)->order('nav_id','asc')->select();
        $this->assign('doc',$doc);
        ...
        $news = db('nav')->where('category_id', 7)->limit(3)->order('nav_id','desc')->select();
    ...
    public function lists_nav(){
        ...
        $det=db('nav')->where('category_id',$category_id)->find();
        $this->assign('det',$det);
